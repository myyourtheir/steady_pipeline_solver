...
def pipeline_traverse(
    ...
    while index >= 0:
        ...
        for withdrawal in withdrawals:
            if x >= withdrawal.position:
                current_Q += withdrawal.flow
        ...
        if index in nps_vsas_indexes:
            current_NPS = [nps for nps in NPS_list if nps.position == profile_x[index]][
                0
            ]
...
def find_gravity_sections(profile_x, profile_z, p_list, H_list, Q, visc_list):
    ...
    for index in range(profile_x.size):
        ...
        is_prev_gravity_index = (
            prev_p == C.vapor_pressure and prev_H == prev_z + prev_p / ro / g
        )

        if not is_prev_gravity_index and is_gravity_index:
            gravity_sections.append(
                {
                    "start": profile_x[index],
                    "start_z": z,
                    "end": profile_x[index],
                    "end_z": z,
                }
    ...
    for index, gravity_section in enumerate(gravity_sections):
        ...
        phi = bisection_method(0.001, math.pi, f)
...
if __name__ == "__main__":
    ...
    if isSuccess:
        ...
        gravity_sections = find_gravity_sections(
            H_list=H_list,
            profile_x=profile_x,
            p_list=p_list,
            profile_z=profile_z,
